...
// ---------------------------------------------------------------------
// AppleSMC struct-call plumbing. Ported from the reference implementation in
// notes/THERMAL_KNOWLEDGE.md (itself ported from iStats' ext/osx_stats/smc.c).
// This is a raw binary struct call into a private IOKit user client — there
// is no public header for any of this.
// ---------------------------------------------------------------------
...
// pLimitData is 16 bytes (UInt16 version, UInt16 length, UInt32 cpuPLimit,
// UInt32 gpuPLimit, UInt32 memPLimit) per Apple's real SMCKeyData_t layout.
// A 14-byte char array here is a known-wrong layout that silently breaks
// every subsequent field in the struct — do not shrink this back to 14.
typedef struct {
    UInt32               key;
    char                 vers[6];
    char                 pLimitData[16];
...
// an IEEE-754 float, matching iStats' _flttof() used for the "flt " type.
...
// SMCGetTemperature returns 0.0 if the key doesn't exist, isn't a
// temperature-shaped value, or the read otherwise fails. Callers must apply
// a plausibility filter — a missing/wrong-type key silently returns 0.0
...
// cpuSMCKeys are the per-chip-generation Apple Silicon CPU SMC keys, sourced
// from notes/THERMAL_KNOWLEDGE.md (a verified-working Swift/IOKit
// implementation and exelban/stats' Modules/Sensors/values.swift).
...
// gpuSMCKeys are the per-chip-generation Apple Silicon GPU SMC keys, from
// notes/THERMAL_KNOWLEDGE.md (crediting btop issue #1653 / exelban/stats).
...
// batterySMCKeys are the per-cell battery temperature SMC keys (TB0T/TB1T
// for a single/dual-cell pack, TB2T for a third cell on larger packs), from
// notes/SMC_BATTERY_SSD.md. Unlike CPU/GPU dies, a battery pack can
// plausibly read below 20°C in a cold environment, so these are probed with
// a wider (0, 80) range filter instead of the (20, 150) used for CPU/GPU.
static const char *batterySMCKeys[] = {
    "TB0T", "TB1T", "TB2T",
};
static const int batterySMCKeysCount = sizeof(batterySMCKeys) / sizeof(batterySMCKeys[0]);

// ssdSMCKeys are the classic Intel/T2-era NVMe blade temperature SMC keys
// (TH0P plus up to 4 individual blade sensors TH0a-TH0d), from
// notes/SMC_BATTERY_SSD.md. Confirmed to still work on at least one Apple
// Silicon Mac (TH0a/TH0b returned plausible readings closely matching the
// IOHID "NAND CH0 temp" node), so the integrated storage controller
// apparently still exposes these SMC keys even without a discrete NVMe
// blade. Not every key is guaranteed to exist on a given machine; if none
// return a reading, the IOHID "NAND CH0 temp" node (HidInfo.nand) is the
// fallback.
...
// smcMaxOfKeysInRange opens the SMC once, reads every key in keys/keyCount,
// and returns the maximum of the readings falling in the exclusive
// (minC, maxC) range — the hottest core/die is treated as the most
// representative single-number reading, per notes/THERMAL_KNOWLEDGE.md.
// Returns {false, 0} if the SMC connection cannot be opened or no key in
// the list yields a reading in range; the AppleSMC struct-call path is
// all-or-nothing (either every key works or every key returns
// kIOReturnNotPrivileged), so a single open covers the whole list.
...
// ---------------------------------------------------------------------
// IOHIDEventSystemClient plumbing. Ported from notes/THERMAL_KNOWLEDGE.md
// (itself ported from macmon's IOHIDSensors). This is part of the public
// IOKit.framework and needs no special privileges, but none of these
// functions ship public headers, so they are hand-declared here.
// ---------------------------------------------------------------------
...
// enumerated more than once — see Gotcha 1 in notes/THERMAL_KNOWLEDGE.md),
// and its reading.
...
// share a LocationID into a single per-node average (Gotcha 1 in
// notes/THERMAL_KNOWLEDGE.md), so a node enumerated more than once isn't
// over-weighted relative to one that appears only once. Readings with no
// LocationID property are each treated as their own node. Returns
// {false, 0} if nothing matched.
...
// not universal — see Gotcha 2 in notes/THERMAL_KNOWLEDGE.md. "PMU tdie*"
...
// ---------------------------------------------------------------------
// Thermal pressure level, via the public Darwin notification API. Ported
// from notes/NOTIFY_THERMAL_STATE.md.
// ---------------------------------------------------------------------

// getThermalPressureLevel returns one of the 5 raw kOSThermalPressureLevel*
// values (0=Nominal, 1=Moderate, 2=Heavy, 3=Trapping, 4=Sleeping) read from
// "com.apple.system.thermalpressurelevel", or {false, 0} if either the
