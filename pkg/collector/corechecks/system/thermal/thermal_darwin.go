...
// AppleSMC and IOHIDEventSystemClient utility in thermal_darwin.c. See
// notes/THERMAL_KNOWLEDGE.md for why both sources are needed and their
// respective failure modes.
type thermalCheck struct {
	core.CheckBase
}

// smcReading is the AppleSMC-sourced half of a thermal sensor snapshot, with
// nil for any sensor that was unavailable or failed to read.
...
type thermalReading struct {
	...
	// 1=Moderate, 2=Heavy, 3=Trapping, 4=Sleeping), nil if the lookup
	// failed. See notes/NOTIFY_THERMAL_STATE.md.
	thermalLevel *int
}
...
// optionalFloat converts a C.OptionalFloat into a *float64, nil when the
// sensor was unavailable or its read failed.
...
// optionalInt converts a C.OptionalInt into a *int, nil when the value was
...
// thermalPressureLevelName maps a raw kOSThermalPressureLevel* value to its
// name, per notes/NOTIFY_THERMAL_STATE.md. "unknown" covers any value
// outside the documented 0-4 range, since the notification is a private,
// undocumented API with no stability guarantee.
