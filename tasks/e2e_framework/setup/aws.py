...
def _aws_create_keypair(
    ...
    # AWS holds the keypair from here on, but it is only usable in combination with the local
    # files built below, and every one of those steps can fail. Leaving the remote half behind
    # strands the keypair in a state no later run can repair on its own -- setup_aws_config can
    # only refuse to continue and print a manual delete command -- so unwind it on any failure.
    # Files that already existed are not ours to remove.
    ...
    try:
        ...
        # BaseException so that interrupting a prompt mid-setup rolls back too.
...
def _rollback_created_keypair(
    ...
    Nothing in here raises. The caller is already unwinding the failure that actually
    matters, and replacing it with a cleanup error would hide the real diagnosis.
    """
    delete_cmd = get_aws_cmd(
        f'ec2 delete-key-pair --key-name "{keypair_name}"',
        use_aws_vault=use_aws_vault,
    ...
    if not deleted:
        ...
        # and are worth more than a tidy directory -- leave them for the user to salvage.
        warn(
